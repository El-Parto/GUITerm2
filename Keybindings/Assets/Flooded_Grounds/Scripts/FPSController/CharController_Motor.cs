...
public class CharController_Motor : MonoBehaviour {
	...
	public float sensitivity = 11555000.0f;
	...
	/// <summary>
	/// Enables or disables the Curso and also changes its visibility.
	/// </summary>
	/// <param name="_active">Whether or not the cursor should be enabled.</param>
	public void SetCursorState(bool _active)
    {
		//if not active, cursor doesn't work
		cam.GetComponent<Camera>().enabled = !_active;
		
		Cursor.visible = _active;
        if (_active)
        {
			// The state of the cursor. Locked means it can't move from thecenter of the screen
